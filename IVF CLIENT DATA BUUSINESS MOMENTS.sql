USE ivf_database;
SHOW COLUMNS FROM ivf_equipment_utilization;
...
/*MEAN FOR  max_capacity_hrs*/
SELECT
    AVG(max_capacity_hrs) AS mean_max_capacity__hours,
    AVG(technical_downtime_hrs) AS mean_technical_downtime_hours,
    AVG(workflow_delay_events) AS mean_workflow_delay_events,
    AVG(total_cases_day_lab) AS mean_total_cases_day_lab
FROM ivf_equipment_utilization;


/*MEDIAN FOR  max_capacity_hrs*/
...
/*MEDIAN FOR  technical_downtime_hrs*/
WITH ordered_data AS (
    SELECT 
        technical_downtime_hrs,
        ROW_NUMBER() OVER (ORDER BY technical_downtime_hrs) AS rn,
        COUNT(*) OVER () AS total_rows
    FROM ivf_equipment_utilization
)
...
/*MEDIAN FOR total_cases_day_lab*/
WITH ordered_data AS (
    SELECT 
        total_cases_day_lab,
...
/*MEDIAN FOR workflow_delay_events*/
WITH ordered_data AS (
    SELECT 
        workflow_delay_events,
        ROW_NUMBER() OVER (ORDER BY workflow_delay_events) AS rn,
        COUNT(*) OVER () AS total_rows
    FROM ivf_equipment_utilization
)
SELECT AVG(workflow_delay_events) AS median_workflow_delay_events
FROM ordered_data
WHERE rn IN (
    FLOOR((total_rows + 1) / 2),
    FLOOR((total_rows + 2) / 2)
...
/*Mode for max_capacity_hrs*/
SELECT 
    max_capacity_hrs AS mode_max_capacity_hrs
FROM ivf_equipment_utilization
GROUP BY max_capacity_hrs
ORDER BY COUNT(*) DESC
LIMIT 1;

/*Mode for technical_downtime_hrs*/
SELECT
	technical_downtime_hrs AS mode_technical_downtime_hrs
FROM ivf_equipment_utilization
GROUP BY technical_downtime_hrs
ORDER BY COUNT(*) DESC
LIMIT 1;

/*Mode for workflow_delay_events*/
SELECT 
	workflow_delay_events AS mode_workflow_delay_events
    FROM ivf_equipment_utilization
    GROUP BY workflow_delay_events
    ORDER BY COUNT(*) DESC
    LIMIT 1;
    
/*MODE FOR total_cases_day_lab*/
SELECT
	total_cases_day_lab AS MODE_total_cases_day_lab
    FROM ivf_equipment_utilization
...
/**********************************************************************************/
/****3 MOMENT**********/
/*Variance of max_capacity_hrs*/
SELECT 
	var_samp(max_capacity_hrs) AS Variance_max_capacity_hrs
    FROM ivf_equipment_utilization;

/*Variance of technical_downtime_hrs*/
SELECT
	var_pop(technical_downtime_hrs) AS variance_technical_downtime_hrs
    FROM ivf_equipment_utilization;

/*Variance of workflow_delay_events*/
SELECT
	var_pop(workflow_delay_events) AS Variance_workflow_delay_events
    FROM  ivf_equipment_utilization;
    
/*Variance of total_cases_day_lab*/
SELECT
	var_pop(total_cases_day_lab) AS Variance_total_cases_day_lab
    FROM  ivf_equipment_utilization;

/*standard deviation of max_capacity_hrs*/
SELECT 
	stddev(max_capacity_hrs) AS sd_max_capacity_hrs
    FROM ivf_equipment_utilization;

/*Standard Deviation  of technical_downtime_hrs*/
SELECT
	stddev(technical_downtime_hrs) AS sd_technical_downtime_hrs
    FROM ivf_equipment_utilization;
    
/*Standard Deviation  of workflow_delay_events*/
SELECT
	stddev(workflow_delay_events) AS sd_workflow_delay_events
    FROM  ivf_equipment_utilization;

/*Standard Deviation of total_cases_day_lab*/
SELECT
	stddev(total_cases_day_lab) AS sd_total_cases_day_lab
    FROM  ivf_equipment_utilization;
/**********************************************************************************/
...
WITH stats AS (
    SELECT
        AVG(total_cases_day_lab) AS mean_val,
        STDDEV_POP(total_cases_day_lab) AS std_val
    FROM ivf_equipment_utilization
),
median_val AS (
    SELECT AVG(total_cases_day_lab) AS median_val
    FROM (
        SELECT
            total_cases_day_lab,
            ROW_NUMBER() OVER (ORDER BY total_cases_day_lab) AS rn,
...
SELECT
    3 * (stats.mean_val - median_val.median_val) / stats.std_val
    AS skewness_total_cases_day_lab
FROM stats, median_val;
/**********************************************************************************/
...
SELECT
(
    SUM(POW(total_cases_day_lab - ANY_VALUE(stats.mean_val), 4)) / COUNT(*)
) / POW(ANY_VALUE(stats.std_val), 4) - 3
AS kurtosis_total_cases_day_lab
FROM ivf_equipment_utilization
JOIN (
    SELECT
        AVG(total_cases_day_lab) AS mean_val,
